...
unsigned long beginPauseTime = 0;
...
void inputGUI(){
  while (Serial1.available() > 0 && lastInput != 'c'){
    ...
    {
      if(hitJunction){
        beginPauseTime = millis() - pauseTime;
      }
      motors.setSpeeds(0, 0);
...
bool checkCorner()
{
  ...
  if (sensorValues[0] >= calibratedValue[0] || sensorValues[2] >= calibratedValue[2])
  {
    ...
    if (sensorValues[1] >= calibratedValue[1])
    {
      ...
      Serial1.print("If its corner press button Corner\nIf its T junction then press T-Junction!\n");
      char cTJunction;
      do{
        cTJunction = (char) Serial1.read();
      }while (!(cTJunction == 'o' || cTJunction == 'j'));
      if(cTJunction == 'j'){
        Serial1.print("Which direction to turn!\n");
        zumoMove();
        hitJunction = true;
        startTime = millis();
...
void personFoundMessage()
{
  ...
  Serial1.println(message + roomCounter);
  // Serial1.print(STRING_TERMINATOR);
  delay(10);
